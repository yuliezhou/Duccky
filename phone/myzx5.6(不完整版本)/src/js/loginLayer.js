...
(function(global, undefined) {
    var num, CountdownSetInter, loginLayer;
    var _global;;
    var html =  '<div class="head_topbar active">'
                +          '<a class="head_left">'
                +              '<i><img src="../../images/pullToLeft.png" alt="" class="back" /></i>'
                +           '</a>'
                ...
                +      '</div>'
                +'<div class="logo">'
                +    '<img src="../../images/myzxlogo.png" alt="">'
                +'</div>'
                +'<div id="loginForm">'
                +        '<div class="phone">'
                ...
                +            '<div class="yz_box">'
                +                '<div class="yz_send" id="yz_send">发送验证码</div>'
                +           ' </div>'
                +        '</div>'
                +        '<a href="##" style="display:block;"><p class="xieyi">点击“登录”即表示同意《名医在线用户服务协议》</p></a>'
                +        '<div class="box">'
                +            '<input type="button" value="登录" class="queding" id="LoginSubmit" data-status = "yz">'
                +        '</div>'
                +    '<div class="otherLoginMeth">'
                +        '<a class="loginMethLeft" id="registerLogin">注册新用户</a>'
                +        '<a class="loginMethRight" id="pwdLogin">密码登录</a>'
                +    '</div>'
                +'</div>';
    ...
    var login = function() {
        ...
        layer.open({
            type: 1,
            content: html, //'可传入任何内容，支持html。一般用于手机页面中'
            anim: true,
            style: 'position:fixed; width: 100%;height: 100%;left:0%; top:0%; padding:0px; border:none;  background-color: #ffffff;',
            success:function(){
                console.log(1);
                clickBind();
            }
...
var strPwd = function(str){
    ...
    var clickBind = function(){
        ...
        $("#registerLogin").click(function() {
            ...
            $(".head_title").html("<font>注册</font>");
        });
        $("#pwdLogin").click(function() {
            reloadLoginForm("pwd");
            $(".head_title").html("<font>登录</font>");
        });
...
//方法调用
    ...
    !('loginLayer' in _global) && (_global.loginLayer = loginLayer);
})(window);